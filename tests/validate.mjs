...
check(banks.length === 13, "題庫必須正好載入 103–115 十三個學年度");
check(banks.map(bank => bank.year).join(",") === "115,114,113,112,111,110,109,108,107,106,105,104,103", "題庫年份必須為 103–115");
...
for (const bank of banks) {
  ...
  for (const q of bank.questions) {
    ...
    if (q.written) {
      ...
      if (bank.year === 103 || bank.year === 104) {
...
console.log(
  `VALIDATE=PASS years=115,114,113,112,111,110,109,108,107,106,105,104,103 questions=${totalQuestions} choices=${totalChoices} ` +
